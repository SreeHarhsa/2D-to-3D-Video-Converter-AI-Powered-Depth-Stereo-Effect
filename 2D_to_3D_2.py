...
class OptimizedVideo3DConverter:
    ...
    def vectorized_stereoscopic_pair(self, frame, depth_map):
        ...
        left_x = np.arange(width).reshape(1, -1) - disparity_map
        right_x = np.arange(width).reshape(1, -1) + disparity_map
        
        # Repeat for all rows
        left_x = np.repeat(left_x, height, axis=0)
        right_x = np.repeat(right_x, height, axis=0)
        
        # Clip coordinates to valid range
        left_x = np.clip(left_x, 0, width-1).astype(np.float32)
        right_x = np.clip(right_x, 0, width-1).astype(np.float32)
        
        # Create y-coordinates (unchanged)
        ...
        for c in range(frame.shape[2]):  # For each color channel
            left_view[:,:,c] = cv2.remap(frame[:,:,c], left_x, y_coords, cv2.INTER_LINEAR)
            right_view[:,:,c] = cv2.remap(frame[:,:,c], right_x, y_coords, cv2.INTER_LINEAR)
